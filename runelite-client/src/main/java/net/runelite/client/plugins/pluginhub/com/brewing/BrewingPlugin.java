package net.runelite.client.plugins.pluginhub.com.brewing;
...
public class BrewingPlugin extends Plugin {
	...
	public static final String ALDARIN_NAME = "Aldarin";
	private static final int ALDARIN_VAT_VARBIT = 11383;
	private static final int ALDARIN_STUFF_VARBIT = 11409;
	private static final int ALDARIN_BARREL_VARBIT = 11384;
	...
	public void onVarbitChanged(VarbitChanged varbitChanged) {
		...
		if ((var == KELDAGRIM_VAT_VARBIT || var == PORT_PHASMATYS_VAT_VARBIT || var == ALDARIN_VAT_VARBIT) &&
				(BrewingVatState.isCompletedNormal(varbitChanged.getValue()) ||
				 BrewingVatState.isCompletedMature(varbitChanged.getValue())))
		{
			String name = "";
			switch (var) {
				case KELDAGRIM_VAT_VARBIT:
					name = KELDAGRIM_NAME;
					break;
				case PORT_PHASMATYS_VAT_VARBIT:
					name = PORT_PHASMATYS_NAME;
					break;
				case ALDARIN_VAT_VARBIT:
					name = ALDARIN_NAME;
					break;
			}
		...
		if (var == KELDAGRIM_VAT_VARBIT || var == PORT_PHASMATYS_VAT_VARBIT || var == ALDARIN_VAT_VARBIT ||
				var == KELDAGRIM_STUFF_VARBIT || var == PORT_PHASMATYS_STUFF_VARBIT || var == ALDARIN_STUFF_VARBIT ||
				var == KELDAGRIM_BARREL_VARBIT || var == PORT_PHASMATYS_BARREL_VARBIT || var == ALDARIN_BARREL_VARBIT) {
			removeInfoBoxes();
			addInfoBoxes();
		}
	}
	...
	private void addInfoBoxes()
	{
		...
		BufferedImage ALDARIN_VAT_IMAGE = client.getVarbitValue(ALDARIN_STUFF_VARBIT) == 1 ? THE_STUFF_VAT_IMAGE : VAT_IMAGE;
		...
		infoBoxManager.addInfoBox(new BrewingVat(ALDARIN_NAME, client.getVarbitValue(ALDARIN_VAT_VARBIT), ALDARIN_VAT_IMAGE, this, config));
		...
		infoBoxManager.addInfoBox(new BrewingBarrel(ALDARIN_NAME, client.getVarbitValue(ALDARIN_BARREL_VARBIT), BARREL_IMAGE, this, config));
	}
